...
import {fetchAuth, login} from "../store/actions/authActions";
...
export const AuthPage = () => {
	...
	const submitHandler = async (e: React.FormEvent<HTMLFormElement>) => {
		try {
			...
			if (isFormValid()) {
				await dispatch(fetchAuth({username: userNameInput, password: passwordInput}))
				navigate('/')
			} else {
	...
	const loginHandler = async (e: React.MouseEvent<HTMLButtonElement, MouseEvent>) => {
		if (isFormValid()) {
			await dispatch(login({username: userNameInput, password: passwordInput}))
			navigate('/')
		} else {
			console.log(`inside if (isFormValid()) 2st`)
		}
		...
		<div className={'mt-28 mx-auto min-h-screen w-screen  max-w-[380px]'}>
			<div className="mockup-code border bg-gray-800 relative text-center ">
				...
				<form action="" onSubmit={submitHandler} className={'border m-2 p-2 rounded'}>
					...
					{/*Register Button:*/}
					{!ToLogIn &&
                        <button type='submit' className='drawer-button border mt-2 p-1 rounded bg-green-700'>Sign
                            Up</button>}
